...
A Python implementation of the Recursive Language Model paradigm from MIT CSAIL research ([arXiv:2512.24601](https://arxiv.org/abs/2512.24601)).
...
### Using Custom Callback (Claude Max / Claude Code)
...
| `CallbackBackend` | Custom integration (Claude Max, etc.) |
...
**Query:** "What authentication methods are discussed?"

**Iteration 1:** LLM writes:
```python
print(f"Context size: {len(CONTEXT):,} chars")
headers = re.findall(r'^## .+$', CONTEXT, re.MULTILINE)
print(f"Sections: {headers}")
```

**Iteration 2:** LLM sees output, writes:
```python
auth_terms = ['OAuth', 'JWT', 'Session', 'MFA']
for term in auth_terms:
    count = len(re.findall(rf'(?i){term}', CONTEXT))
    print(f"{term}: {count} mentions")
```

**Iteration 3:** LLM chunks and analyzes:
```python
oauth_section = re.search(r'## OAuth.*?(?=## |$)', CONTEXT, re.DOTALL).group()
analysis = llm_query(f"Summarize security considerations:\n{oauth_section}")
print(analysis)
```

**Iteration 4:** LLM synthesizes:
```python
FINAL("""
...
MIT License - Based on research by Alex L. Zhang, Tim Kraska, and Omar Khattab (MIT CSAIL).
